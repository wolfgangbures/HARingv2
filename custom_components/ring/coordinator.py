...
class RingDataCoordinator(DataUpdateCoordinator[RingDevices]):
    ...
    async def _call_api[*_Ts, _R](
        ...
        *args: *_Ts,
        ...
        try:
            self.logger.debug("Calling Ring API: %s", getattr(target, "__name__", target))
    ...
    async def _async_update_data(self) -> RingDevices:
        ...
        self.logger.debug(
            "Polling Ring devices using %s (first_call=%s)",
            update_method,
            self.first_call,
        )
...
class RingListenCoordinator(BaseDataUpdateCoordinatorProtocol):
    ...
    def index_alerts(self) -> None:
        ...
        self.logger.debug("Indexing Ring alerts")
        self.alerts = {
            (alert.doorbot_id, alert.kind): alert
            for alert in self.ring_api.active_alerts()
        }
        self.logger.debug("Indexed %s Ring alerts", len(self.alerts))
    ...
    async def _async_stop_listen(self) -> None:
        self.logger.debug("Stopping Ring event listener")
    ...
    async def _async_start_listen(self) -> None:
        ...
        self.logger.debug("Registering Ring event callback")
    ...
    def async_add_listener(
        ...
        self.logger.debug("Adding listener (context=%s). Existing=%s", context, len(self._listeners))
        ...
        def remove_listener() -> None:
            ...
            if not self._listeners:
                self.logger.debug("Last listener removed, stopping event listener")
