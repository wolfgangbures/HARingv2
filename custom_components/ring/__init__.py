...
async def async_setup_entry(hass: HomeAssistant, entry: RingConfigEntry) -> bool:
    ...
    _LOGGER.debug(
        "Setting up Ring config entry %s (user=%s)",
        entry.entry_id,
    ...
    ring = Ring(auth)
    _LOGGER.debug("Ring API initialized for entry %s", entry.entry_id)
    ...
    _LOGGER.debug(
        "Coordinators created (devices/listen) for entry %s", entry.entry_id
    )

    _LOGGER.debug("Performing initial device refresh for entry %s", entry.entry_id)
...
async def async_unload_entry(hass: HomeAssistant, entry: RingConfigEntry) -> bool:
    """Unload Ring entry."""
    _LOGGER.debug("Unloading Ring entry %s", entry.entry_id)
